...
int string_bst::word_frequency(const tree_key &key) const {
	...
	if (root->data.word == key) {
		...
		return word_frequency_helper(root->left, key);
	} else {
		return word_frequency_helper(root->right, key);
	}
...
int string_bst::word_frequency_helper(node_t * n, tree_key const& key) const {
	if (!n) return 0;
	if (n->data.word == key) return n->data.freq;
	else if (key < n->data.word) return word_frequency_helper(n -> left, key);
